...
# Configure Logging for Vercel (Write to /tmp if local fails)
log_filename = "/tmp/abiba.log" if os.environ.get('VERCEL') else "abiba.log"

# Setup basic logging to stdout (which Vercel captures)
logging.basicConfig(
    level=logging.INFO,
...
# Attempt file logging only if we have write access
try:
    file_handler = logging.FileHandler(log_filename)
    logging.getLogger().addHandler(file_handler)
except Exception:
    logging.warning("File logging disabled (read-only filesystem).")
...
def get_logs():
    try:
        # On Vercel, we can't easily read a log file. Returning recent system messages.
